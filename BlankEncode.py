...
# 默认字典
default_dict = [chr(ch) for ch in range(0x2000, 0x2010)] # 这些字符全为不可见的，有的是特殊空格，有的是 Unicode 操纵字符
encodeSuffix = ""
decodeSuffix = ""

def is_valid_dict(dicto:list):
    unique_dict = list(set(dicto))
    return len(unique_dict) == len(dicto)

def encode(input:bytes, dicto:list):
    encoded_str = ""
    for b in input:
        encoded_str += dicto[b>>4]  # 先转码二进制的高四位，对应2位16进制数的第一位
        encoded_str += dicto[b&0xf] # 再转码二进制的低四位，对应2位16进制数的第二位

    return encoded_str


def decode(input:str, dicto:list):
    decoded_bytes = bytes()
    try:
        for i in range(0, len(input), 2):
            h = dicto.index(input[i])    
            l = dicto.index(input[i+1])  # 从字典查找并且拼凑出原先的字节
            decoded_bytes += bytes([(h<<4)|l])
    except ValueError: 
        print("字典与加密内容不匹配！")
        return bytes([])
    else: return decoded_bytes
...
def transcode_file(dictPath:str, encodePath:str, decodePath:str):
    ...
    try:
        if(len(dictPath) == 0): dicto = default_dict # 未指定字典时采用默认的“空白字符”字典
        else:
        ...
        if(len(encodePath) == 0):
            mode = 0
            encodePath = "result.enc."+encodeSuffix
        if(len(decodePath) == 0):
            if(mode == 0): sys.exit(4)
            mode = 1
            decodePath = "result.dec."+decodeSuffix
...
字典内容应为无重复字符且长度为 16 的字符串。如果字典未指定，采用默认空白字典。")
    ...
    for i in range(1, argc):
        ...
        if(splited.count("enc")):
            encodePath = argv[i]
            decodeSuffix = splited[splited.index("enc") - 1]
        if(argv[i].split('.').count("dec")):
            decodePath = argv[i]
            encodeSuffix = splited[splited.index("dec") - 1]
